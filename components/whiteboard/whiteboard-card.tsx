...
export function WhiteboardCard({
  ...
  if (viewMode === "list") {
    return (
      <motion.div
        ...
        onClick={handleOpen}
        className="group flex items-center gap-4 p-4 bg-surface border border-border rounded-lg cursor-pointer transition-all hover:bg-surface-hover"
      >
        {/* Icon */}
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-foreground truncate group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors">
            {whiteboard.name}
          </h3>
        ...
        <button
          ...
          disabled={isDeleting}
          className="opacity-0 group-hover:opacity-100 p-2 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all disabled:opacity-50"
        >
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      onClick={handleOpen}
      className="group relative bg-surface border-2 border-border rounded-xl p-5 cursor-pointer hover:border-orange-400/50 hover:shadow-xl hover:shadow-orange-500/10 transition-all"
    >
      {/* Delete Button */}
      <button
        onClick={handleDelete}
        disabled={isDeleting}
        className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-2 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all z-10 disabled:opacity-50"
      >
        <Trash2 className="w-4 h-4" />
      </button>
      ...
      {/* Title */}
      <h3 className="font-semibold text-foreground truncate mb-2 group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors">
        {whiteboard.name}
      </h3>
